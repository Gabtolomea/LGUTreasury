...
namespace LGUTreasury.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            ViewBag.PendingRequests = await _context.Editrequests.Where(r => r.Status == "Pending").CountAsync();
            ...
            ViewBag.MtdTotal      = (await _context.PaymentRecords.Where(p => p.DateIssued >= monthStart && p.DateIssued <= today).SumAsync(p => p.TotalAmount)).ToString("N2");
            ...
            var allPayments = await _context.PaymentRecords.Where(p => p.DateIssued.Date >= weekStart && p.DateIssued.Date <= today).ToListAsync();
            var dailyTotals = Enumerable.Range(0, 7).Select(i => allPayments.Where(p => p.DateIssued.Date == weekStart.AddDays(i)).Sum(p => p.TotalAmount)).ToList();
            var maxTotal    = dailyTotals.Max() == 0 ? 1 : dailyTotals.Max();
            ViewBag.BarHeights = dailyTotals.Select(t => (int)Math.Round((t / maxTotal) * 100)).ToList();
            ViewBag.BarLabels  = Enumerable.Range(0, 7).Select(i => weekStart.AddDays(i).ToString("ddd")).ToList();

            // ── Collector Leaderboard ─────────────────────
            // Parse date range from query string, default Jan 1 to today
            var leaderboardFrom = today;
            var leaderboardTo   = today;

            if (DateTime.TryParse(Request.Query["from"].ToString(), out var parsedFrom))
                leaderboardFrom = parsedFrom;
            else
                leaderboardFrom = new DateTime(today.Year, 1, 1);

            if (DateTime.TryParse(Request.Query["to"].ToString(), out var parsedTo))
                leaderboardTo = parsedTo;
            else
                leaderboardTo = today;
